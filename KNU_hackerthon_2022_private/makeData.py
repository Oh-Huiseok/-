...
def make_image(i, photo_ids):
    createFolder('./images/' + str(i))
    for j, raw in enumerate(photo_ids):
        raw_image_data = gmaps.places_photo(
            photo_reference=raw, max_height=maxheight, max_width=maxwidth)
        filename = "images/%d/%d-%d.jpg" % (i, i, j)
        print(filename)
        f = open(filename, 'wb')
...
def parse(item, i):
    ...
    try:
        ...
        photo_ids = []
        for ref in place_details['result']['photos']:
            photo_ids.append(ref['photo_reference'])
        make_image(i, photo_ids)
        PHOTO_AVAIL = True
    except:
        PHOTO_AVAIL = False
